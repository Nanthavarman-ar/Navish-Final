...
const AIStructuralAdvisor: React.FC<AIStructuralAdvisorProps> = ({
  ...
  useEffect(() => {
    const unsubscribe = networkMonitor.addListener((online) => {
      setIsOnline(online);
      if (online && isAnalyzing) {
        // Resume analysis if we come back online
        runAnalysis();
      }
    });
  ...
  // Enhanced AI Analysis Engine with error handling
  const performStructuralAnalysis = useCallback(async (): Promise<StructuralAnalysis[]> => {
    ...
    try {
      ...
      if (meshes.length === 0) {
        throw new Error('No valid meshes found for analysis');
      }
      ...
      for (let i = 0; i < meshes.length; i++) {
        ...
        try {
          ...
          console.warn(`Analysis failed for mesh ${mesh.name}:`, meshError);
      ...
      throw new Error(`Analysis engine error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }, [scene]);
  ...
  const analyzeLoadBearing = (mesh: Mesh): StructuralAnalysis | null => {
    try {
      ...
      console.warn('Load bearing analysis failed:', error);
      return null;
    }
  };
  ...
  const analyzeMaterial = (mesh: Mesh): StructuralAnalysis | null => {
    try {
      ...
      console.warn('Material analysis failed:', error);
      return null;
    }
  };
  ...
  const analyzeConnections = (mesh: Mesh): StructuralAnalysis | null => {
    try {
      ...
      console.warn('Connection analysis failed:', error);
      return null;
    }
  };
  ...
  const calculateSurfaceArea = (mesh: Mesh): number => {
    try {
      ...
      console.warn('Surface area calculation failed:', error);
      return 0;
    }
  };
  ...
  const runAnalysis = useCallback(async () => {
    if (!scene || isAnalyzing || !isOnline) return;
    ...
    try {
      ...
      const result = await retryNetworkOperation(
        () => performStructuralAnalysis(),
        {
          ...
          onRetry: (attempt, error) => {
            console.log(`Analysis retry attempt ${attempt}:`, error.message);
      ...
      if (result.success && result.data) {
        ...
        const calculatedMetrics = calculateOverallMetrics(result.data);
        setMetrics(calculatedMetrics);

        // Add to history
        setAnalysisHistory(prev => [result.data!, ...prev.slice(0, 9)]); // Keep last 10 analyses
  ...
  }, [scene, isAnalyzing, isOnline, performStructuralAnalysis, calculateOverallMetrics, onAnalysisComplete]);
  ...
  useEffect(() => {
    if (isRealTimeMode && isActive && isOnline) {
  ...
  }, [isRealTimeMode, isActive, isOnline, runAnalysis]);

  // Auto-run analysis when component becomes active
  useEffect(() => {
    if (isActive && analyses.length === 0 && isOnline) {
      runAnalysis();
    }
  }, [isActive, analyses.length, isOnline, runAnalysis]);
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                ...
                {!isOnline && <div className="w-2 h-2 bg-red-500 rounded-full ml-2" title="Offline" />}
              </CardTitle>
            ...
            {!isOnline && (
              <Alert className="border-orange-200 bg-orange-50">
                ...
                <AlertDescription className="text-orange-700">
                  AI features require an internet connection. Analysis will resume when connection is restored.
                </AlertDescription>
              </Alert>
            )}
              ...
              >
                {isAnalyzing ? (
                  <>
                    ...
                    Analyzing... {retryCount > 0 && `(Retry ${retryCount})`}
                  </>
                ) : (
                  <>
