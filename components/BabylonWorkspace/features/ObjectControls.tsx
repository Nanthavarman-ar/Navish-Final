...
const OBJECT_PRESETS: Record<ObjectType, {
  name: string;
  icon: React.ComponentType<{ className?: string }>;
  description: string;
  defaultProperties: Omit<ObjectProperties, 'id' | 'name' | 'type' | 'visible' | 'locked' | 'tags' | 'metadata'>;
}> = {
  box: {
    name: 'Box',
    icon: Box,
...
export function ObjectControls({
  ...
  const handleObjectUpdate = useCallback((objectId: string, properties: Partial<ObjectProperties>) => {
    setObjects(prev => prev.map(obj =>
      obj.id === objectId ? { ...obj, ...properties } as ObjectProperties : obj
    ));
