...
export function CameraControls({
  ...
  const currentRotation = (state.scene?.activeCamera as any)?.rotation || { x: 0, y: 0, z: 0 };
  ...
  const handlePositionChange = useCallback((axis: 'x' | 'y' | 'z', value: number) => {
    const newPosition = new Vector3(
      axis === 'x' ? value : currentPosition.x,
      axis === 'y' ? value : currentPosition.y,
      axis === 'z' ? value : currentPosition.z
    );
  ...
  const handleRotationChange = useCallback((axis: 'x' | 'y' | 'z', value: number) => {
    const newRotation = new Vector3(
      axis === 'x' ? value : currentRotation.x,
      axis === 'y' ? value : currentRotation.y,
      axis === 'z' ? value : currentRotation.z
    );
  ...
  const handlePresetSelect = useCallback((presetId: string) => {
    ...
    if (preset) {
      ...
      onPositionChange?.(new Vector3(preset.position.x, preset.position.y, preset.position.z));
      onRotationChange?.(new Vector3(preset.rotation.x, preset.rotation.y, preset.rotation.z));
