...
const FoodExpiryTracker: React.FC<FoodExpiryTrackerProps> = ({ isActive, onClose }) => {
  ...
  const handleBarcodeDetected = (barcode: string, barcodeFormat: string) => {
    // Auto-fill form with common barcode data
    const commonItems: Record<string, Partial<FoodItem>> = {
      'DEMO123456': {
        name: 'Demo Milk 2L',
        category: 'dairy',
        quantity: 1,
        unit: 'pieces',
        expiryDate: format(addDays(new Date(), 14), 'yyyy-MM-dd') as string,
      ...
      'DEMO234567': {
        ...
        quantity: 1,
        unit: 'pieces',
        expiryDate: format(addDays(new Date(), 5), 'yyyy-MM-dd') as string,
        storageLocation: 'pantry'
      }
    };
    ...
    const itemData = commonItems[barcode] || {
      ...
      quantity: 1,
      unit: 'pieces' as FoodItem['unit'],
      expiryDate: format(addDays(new Date(), 7), 'yyyy-MM-dd') as string,
      storageLocation: 'pantry' as FoodItem['storageLocation']
    };
