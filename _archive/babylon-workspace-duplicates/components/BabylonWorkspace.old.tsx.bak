...
const BabylonWorkspace: React.FC<{ workspaceId: string }> = ({ workspaceId }) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const [engine, setEngine] = useState<Engine | null>(null);
  const [scene, setScene] = useState<Scene | null>(null);
  const [simulationManager, setSimulationManager] = useState<SimulationManager | null>(null);
  const [sceneManager, setSceneManager] = useState<{ scene: Scene } | null>(null);
  ...
  const [selectedObject, setSelectedObject] = useState<AbstractMesh | null>(null);
      ...
      for (const gp of gamepads) {
        ...
        if (scene.activeCamera) {
          ...
          if ('rotation' in camera) {
            (camera as any).rotation.y += rotateX * cameraRotateSpeed;
            (camera as any).rotation.x += rotateY * cameraRotateSpeed;
          }
        }
      }
  ...
  return (
    <div className="babylon-workspace-container">
      <EnhancedToolbar
        ...
        isVisible={true}
        lightingEnabled={lightingControlEnabled}
      />
      <canvas ref={canvasRef} className="babylon-canvas" />
      {/* Conditional rendering of feature components */}
      {weatherSystemEnabled && <WeatherSystem scene={scene!} />}
      {floodSimulationEnabled && <FloodSimulation scene={scene!} terrainMesh={undefined} />}
      {enhancedFloodEnabled && <EnhancedFloodSimulation scene={scene!} />}
      {windTunnelEnabled && <WindTunnelSimulation scene={scene!} />}
      {noiseSimulationEnabled && <NoiseSimulation scene={scene!} />}
      {shadowAnalysisEnabled && <ShadowImpactAnalysis scene={scene!} engine={engine!} />}
      {trafficParkingEnabled && simulationManager && <TrafficParkingSimulation simulationManager={simulationManager} />}
      {measureToolEnabled && <MeasureTool scene={scene!} engine={engine!} isActive={measureToolEnabled} />}
      {aiAdvisorEnabled && <AIStructuralAdvisor scene={scene!} engine={engine!} isActive={aiAdvisorEnabled} />}
      {circulationFlowEnabled && <CirculationFlowSimulation scene={scene!} engine={engine!} isActive={circulationFlowEnabled} />}
      {ergonomicTestingEnabled && <ErgonomicTesting scene={scene!} />}
      {soundPrivacyEnabled && <SoundPrivacySimulation scene={scene!} />}
      {multiSensoryEnabled && <MultiSensoryPreview scene={scene!} />}
      {lightingMoodsEnabled && <LightingMoodBoardsFixed scene={scene!} />}
      {furnitureClearanceEnabled && <FurnitureClearanceChecker scene={scene!} />}
      {siteContextEnabled && <SiteContextGenerator scene={scene!} />}
      {topographyEnabled && <TopographyGenerator scene={scene!} />}
      {constructionOverlayEnabled && <ConstructionOverlay scene={scene!} />}
      {geoLocationEnabled && <GeoLocationContext scene={scene!} />}
      {aiCoDesignerEnabled && sceneManager && <AICoDesigner sceneManager={sceneManager} onClose={toggleAiCoDesigner} />}
      {autoFurnishEnabled && sceneManager && <AutoFurnish sceneManager={sceneManager} onClose={toggleAutoFurnish} />}
      {sunlightAnalysisEnabled && <SunlightAnalysis scene={scene!} isActive={sunlightAnalysisEnabled} />}
      {bimIntegrationEnabled && <BIMIntegration scene={scene!} isActive={bimIntegrationEnabled} />}
      {costEstimatorEnabled && <CostEstimatorWrapper scene={scene!} />}
      {materialManagerEnabled && <MaterialManagerWrapper scene={scene!} socket={null} userId="user1" />}
      {vrArModeEnabled && <VRARMode scene={scene!} isActive={vrArModeEnabled} />}
      {handTrackingEnabled && <HandTracking scene={scene!} isActive={handTrackingEnabled} />}
      {multiUserEnabled && <MultiUser scene={scene!} isActive={multiUserEnabled} />}
      {voiceAssistantEnabled && <VoiceChat scene={scene!} isActive={voiceAssistantEnabled} />}
      {presenterModeEnabled && <PresenterMode scene={scene!} isActive={presenterModeEnabled} />}
      {annotationsEnabled && <Annotations scene={scene!} />}
      {exportVisible && <ExportTool scene={scene!} />}
      {pathTracingEnabled && <PathTracing scene={scene!} />}
      {iotIntegrationEnabled && <IoTIntegration scene={scene!} />}
      {energyAnalysisEnabled && <EnergySim scene={scene!} />}
      {propertyInspectorEnabled && <PropertyInspector scene={scene!} engine={engine!} selectedObject={selectedObject} />}
      {sceneBrowserEnabled && <SceneBrowser scene={scene!} engine={engine!} />}
      {/* Missing feature components */}
      {minimapEnabled && scene!.activeCamera && <Minimap scene={scene!} camera={scene!.activeCamera} />}
      {materialEditorEnabled && <MaterialEditorPanel selectedMesh={selectedObject} scene={scene!} />}
      {lightingControlEnabled && <LightingPresets scene={scene!} />}
      {movementToolEnabled && scene!.activeCamera && <MovementControlChecker scene={scene!} camera={scene!.activeCamera} />}
    </div>
  );
};
